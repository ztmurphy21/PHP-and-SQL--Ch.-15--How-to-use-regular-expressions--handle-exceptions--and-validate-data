...
class Validate {
    ...
    public function pattern($name, $value, $pattern, $message,
     ...
     public function phone($name, $value, $required = false) {
        ...
        $pattern = '/^\d{3}-\d{3}-\d{4}$/'; //step #4 changed pattern
        $message = 'Invalid phone number.';
        $this->pattern($name, $value, $pattern, $message, $required);
    }
    
    //birthdate function step #7
    ...
    public function password($name, $password, $required = true) {
        ...
        $this->text($name, $password, $required, 8); //step #5, must have 8 characters
        if ($field->hasError()) { return; }
        ...
        $charClasses[] = '[:upper:]';
        $charClasses[] = '[:lower:]'; 
        //$charClasses[] = '_-';  not required as per step #5
    ...
    public function zip($name, $value, $required = true) {
        ...
        $pattern = '/^\d{5}(-\d{4})?$/'; //changed pattern step #4
